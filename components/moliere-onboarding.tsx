...
type Props = {
  onTermine: (categories: string[]) => void
}
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#111', overflowY: 'auto' }}>
      ...
      <div style={{ padding: '20px 18px', flex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          {CATEGORIES.map(cat => {
            ...
            return (
              <button key={cat.id} onClick={() => toggleCategorie(cat.id)} style={{ padding: '14px 12px', borderRadius: '12px', border: estSelectionne ? '2px solid #F5C842' : '2px solid #2A2A2A', background: estSelectionne ? '#1A1500' : '#1C1C1C', cursor: 'pointer', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '6px' }}>
      ...
      <div style={{ padding: '16px 18px 32px' }}>
        <button onClick={valider} style={{ width: '100%', padding: '15px', borderRadius: '14px', border: 'none', background: '#F5C842', color: '#111', fontSize: '16px', fontWeight: 700, cursor: 'pointer' }}>
          {selected.length === 0 ? 'Tout explorer →' : `Valider mes ${selected.length} thème${selected.length > 1 ? 's' : ''} →`}
        </button>
        {selected.length > 0 && (
          <button onClick={() => setSelected([])} style={{ width: '100%', marginTop: '10px', padding: '10px', border: 'none', background: 'none', color: '#A0A0A0', fontSize: '13px', cursor: 'pointer' }}>
            Tout sélectionner
          </button>
        )}
      </div>
